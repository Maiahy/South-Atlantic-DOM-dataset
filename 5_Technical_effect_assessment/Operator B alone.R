...
pcoa_res <- cmdscale(bray_dist, eig = TRUE, k = 2, add = TRUE)
...
library(plotly)
colnames(metadata)
p <- ggplot(pcoa_df, aes(
  PCoA1, PCoA2,
  color = technical_replicate,
  text = paste(
    "File:", file_name,
    "<br>Station:", station,
    "<br>Study:", topical_study
  )
)) +
  
  geom_point(size = 2) +
  stat_ellipse(aes(group = sample_id), alpha = 0.2)+
  geom_path(aes(group = sample_id), alpha = 0.3)

ggplotly(p, tooltip = "text")
table(pcoa_df$sample_id)
...
ggplot(pcoa_df2, aes(PCoA1, PCoA2, color = technical_replicate)) +
  geom_segment(aes(xend = cx, yend = cy), alpha = 0.3) +
  geom_point(size = 2)
...
p <- ggplot(pcoa_df2, aes(
  ...
  geom_segment(
    ...
    alpha = 0.3,
    show.legend = FALSE   # important pour éviter du bruit
  ) +
  geom_point(size = 2)
...
p <- ggplot(pcoa_df2, aes(
  ...
  geom_segment(aes(xend = cx, yend = cy, group = sample_id),
               color = "black",
               alpha = 0.3)+
...
dev.off()
#WHat is the distribution of these outliers in terms of TIC? 
...
metadata_mean <- pca_input_surface %>%
  ...
  summarise(
    ...
    station = first(ESSENTIAL_station_label),
    topical_study = first(project_topical_study_label),
    CTD_temperature_degC_median = first(CTD_temperature_degC_median),
...
p_mean_operator <- ggplot(pcoa_df_mean, aes(
  PCoA1, PCoA2,
  color = CTD_temperature_degC_median  
)) +
  geom_point(size = 2.5, alpha = 0.8) +
  geom_point(size = 2.5, alpha = 0.8) +
...
combined_plot <- (p_mean_evaporation | p_mean_latitude) +
  plot_annotation(tag_levels = "A")
ggsave(
  filename = file.path(
    output_path,
